...
const HomeBody = () => {
  ...
  const searchHandler = (e) => {
    ...
    if (e.target.value === "") {
      filterHandler(
        allJobs ? "all" : userType === "user" ? "my-jobs" : "applied"
      );
      return;
    }
  ...
  const filterHandler = (condition) => {
    ...
    if (condition === "active") {
      ...
      const filtered = jobsArray.filter(
        (job) =>
          ...
          job.jobRequest.some((request) => request.status === "Applied")
      );
      setFilteredJobs(filtered);
    }
    ...
    if (condition === "applied") {
      ...
      const filtered = jobsArray.filter((job) =>
        job.jobRequest.some((request) => request.user.email === email)
      );
      setFilteredJobs(filtered);
    }
  };
  ...
  return (
    <main className="h-5/6">
      <section className="h-24 flex justify-between items-center max-md:flex-col-reverse max-md:my-6 max-md:mx-24 max-sm:mx-6 max-md:h-20">
          ...
          {userType === "user" && (
            <>
              <button
                ...
                className={`w-[90px] justify-center flex items-center px-3 py-2 h-full ${
                  allJobs ? "bg-slate-800 text-white" : ""
                }`}
              >
                Posted
              </button>
              <button
                ...
                className={`w-[90px] flex items-center justify-center px-3 py-2 h-full ${
                  allJobs ? "" : "bg-slate-800 text-white"
                }`}
              >
                Active
        ...
        <div
          id="searchdiv"
          className="mx-24 w-60 h-8 flex items-center border border-gray-600 rounded-full overflow-hidden  max-sm:mx-10 max-md:w-full"
        >
          <input
            type="search"
            id="search"
