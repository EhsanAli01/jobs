...
const CardDetails = () => {
  ...
  const button = twMerge(
    "w-32 py-1 rounded-lg text-white relative font-semibold transition-all duration-200 flex justify-center items-center h-10"
  );
  ...
  return (
    <section>
      <div className="text-gray-800 flex justify-between items-center my-6 mx-24">
        <div
          className="border border-gray-400 text-4xl rounded-full transition-all duration-150 cursor-pointer p-1 bg-slate-100 hover:shadow-xl hover:bg-slate-200"
          onClick={() => navigate("/")}
        >
          <IoMdArrowRoundBack />
        </div>
          ...
          <>
            {!(
              ...
              <button
                ...
                onClick={() => navigate(`/contractor/apply-job/${id}`)}
              >
                Apply
              </button>
            )}
                ...
                return (
                  <span
                    className={`${appliedButton} ${
                      obj.status == "Accepted" ? "bg-green-600" : ""
                    }`}
                  >
                    {obj.status}
                  </span>
        ...
        {userType === "user" &&
          ...
          filteredRequests.length > 0 && (
            ...
            >
              Requests
              <span className="border-2 border-red-600 bg-red-600 rounded-full w-6 h-6 text-sm absolute -top-1 -right-2">
